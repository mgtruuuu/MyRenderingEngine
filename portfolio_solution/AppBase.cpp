...
AppBase::AppBase()
    //: m_screenWidth(3840), m_screenHeight(2160), m_mainWindow(0),
...
void AppBase::OnMouseMove(int mouseX, int mouseY) {
    ...
    // 커서가 화면 밖으로 나갔을 경우 범위 조절 (게임에서는 클램프를 안할 수도 있음)
...
void AppBase::InitCubemaps(wstring basePath, wstring envFilename,
    ...
    // BRDF LookUp Table is 2D Texture, not CubeMap.
...
void AppBase::CreateDepthBuffers() {
    ...
    // Depth 전용
    desc.Format =
        DXGI_FORMAT_R32_TYPELESS; // CreateDepthStencilView() 에 적용되는
                                  // DXGI_FORMAT_D32_FLOAT과
                                  // CreateShaderResourceView()에 적용되는
                                  // DXGI_FORMAT_R32_FLOAT가 타입이 달라서 ...?
